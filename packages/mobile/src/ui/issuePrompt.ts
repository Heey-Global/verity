 ...
 * branch name — the content travels in the prompt). Trusted task instructions lead;
 * the issue title/body are JSON-serialized in the provenance-labelled tail. An
 * empty body is omitted. The instruction keeps the agent on the repo's
 * branch→test→PR workflow and links the PR back to the issue.
 *
...
export function buildIssuePrompt(issue: Pick<IssueSummary, 'number' | 'title' | 'body'>): string {
  const instruction =
    `Work on GitHub issue #${String(issue.number)}. ` +
  ...
  return appendExternalPromptData(instruction, `GitHub issue #${String(issue.number)}`, {
    title: issue.title.trim(),
    ...(issue.body.trim().length > 0 ? { body: issue.body.trim() } : {}),
  });
}
