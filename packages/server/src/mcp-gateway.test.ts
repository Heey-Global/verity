...
it('restricts knowledge bearers during discovery and direct calls, including project extras', async () => {
  const h = harness({
    servedTools: ['verity_knowledge', 'verity_http_request', 'verity_secret_run'],
    extraToolsForProject: () => ['verity_session_handoff'],
    resolveCaller: async () => ({ sessionId: 'sess-1', turnId: 'turn-1', scope: 'knowledge' }),
    hasStandingAuthorization: async () => true,
  });
  const response = await h.gateway.handle({
    projectId: 'p1',
    token: 'knowledge-token',
    body: { jsonrpc: '2.0', id: 1, method: 'tools/list' },
  });
  const names = (response.body as { result: { tools: { name: string }[] } }).result.tools.map(
    (t) => t.name,
  );
  expect(names).toEqual(['verity_knowledge']);
  // Hiding tools in discovery alone leaves direct JSON-RPC calls able to reach secrets.
  for (const name of ['verity_http_request', 'verity_secret_run', 'verity_session_handoff']) {
    const denied = await h.gateway.handle({
      projectId: 'p1',
      token: 'knowledge-token',
      body: call({}, name),
    });
    expect(denied.body).toMatchObject({ error: { message: `unknown tool ${name}` } });
  }
  ...
  await h.gateway.handle({
    projectId: 'p1',
    token: 'knowledge-token',
    body: call({ operation: 'list' }, 'verity_knowledge'),
  });
  expect(h.invokeTool).toHaveBeenCalledWith(
    expect.objectContaining({ toolName: 'verity_knowledge', sessionId: 'sess-1' }),
  );
});
