...
it('a scoped Wiki caller cannot discover or invoke broader tools even in the Control project', async () => {
  const h = harness({
  ...
  const listed = await h.gateway.handle({
    projectId: 'verity-control',
    token: 'session-token',
    body: { jsonrpc: '2.0', id: 1, method: 'tools/list' },
  });
  expect(
    (listed.body as { result: { tools: { name: string }[] } }).result.tools.map(
      (tool) => tool.name,
    ),
  ).toEqual(['verity_knowledge']);
  for (const name of [
    'verity_http_request',
    'verity_secret_run',
    'verity_list_sessions',
    ...
    expect(response.body).toMatchObject({ error: { message: `unknown tool ${name}` } });
  }
  ...
  expect(h.invokeTool).toHaveBeenCalledOnce();
});
