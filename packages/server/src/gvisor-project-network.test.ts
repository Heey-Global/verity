...
import {
  ...
  relayHostEntries,
  sandboxResolvConf,
  upstreamResolversFromDockerResolvConf,
...
// resolves to nothing, and the turn only fails later, at egress, with ENOTFOUND. These cover the
// two inputs the pinning is built from: what Docker reports for the relay, and what it wrote into
// the Server's own resolv.conf.
...
describe('upstream resolvers', () => {
  // Written by Docker 29.3 into a container on a user-defined network, verbatim.
  const dockerResolvConf = [
    ...
    '# ExtServers: [host(8.8.8.8) host(8.8.4.4)]',
    '# Overrides: []',
    '',
  ].join('\n');

  it('reads the servers Docker forwards to, never the embedded 127.0.0.11', () => {
    expect(upstreamResolversFromDockerResolvConf(dockerResolvConf)).toEqual(['8.8.8.8', '8.8.4.4']);
  });

  it('drops loopback upstreams, which would name the Sandbox itself', () => {
    expect(
      upstreamResolversFromDockerResolvConf('# ExtServers: [host(127.0.0.53) host(192.0.2.1)]'),
    ).toEqual(['192.0.2.1']);
    expect(upstreamResolversFromDockerResolvConf('nameserver 1.1.1.1\n')).toEqual([]);
  });
