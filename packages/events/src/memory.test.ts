...
describe('MEMORY_SYSTEM_PROMPT', () => {
  it('gives every fresh context the self-contained memory command', () => {
    ...
    expect(MEMORY_SYSTEM_PROMPT).toMatch(/remember a short fact/i);
    expect(MEMORY_SYSTEM_PROMPT).toContain('/knowledge/insights');
  ...
  it('keeps durable memory concise and safe', () => {
    ...
    expect(MEMORY_SYSTEM_PROMPT).toMatch(/neither secrets nor transient state/i);
  ...
  it('steers agents away from their own backend memory', () => {
    ...
    expect(MEMORY_SYSTEM_PROMPT).toMatch(
      /never save durable project information in your backend's own memory/i,
    );
