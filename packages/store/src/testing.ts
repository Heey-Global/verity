...
export async function truncateAll(db: Kysely<Database>): Promise<void> {
  ...
  await sql`truncate table control_plane_generation, uplink_pending_share_removals, transcript_lines, events, queued_turns, running_turns, runner_frames, session_pending_note, session_automation_marker, sessions, attachments, secret_audit_events, audit_mac_keys, secret_job_frames, secret_jobs, secret_revocations, secret_approvals, secret_run_grants, agent_loop_runs, agent_loops, public_preview_shares, google_slide_image_cleanup, recent_google_slide_decks, dev_server_detection_state, dev_servers, claude_egress_client_certs, claude_egress_ca, project_settings, verity_settings, secret_key_meta, push_receipts, device_push_tokens, auth_tokens, session_backend_state, project_identity_claims, projects restart identity cascade`.execute(
    db,
  );
}
