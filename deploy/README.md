...
- Brokered Secret jobs are currently **amd64-only** and require the pinned gVisor host runtime.
  They are unavailable on arm64 even when `runsc` is installed. On amd64, install and verify it
...
Direct installations may still read the former `VERITY_PARAKEET_*` names as a
deprecated upgrade fallback, but new configuration must use `VERITY_TRANSCRIBE_*`;
the current Server never starts or addresses a Parakeet service.
...
| `VERITY_GVISOR_REQUIRED`               | `0`                                                                  | On amd64, set to `1` on hosts that run Brokered Secret jobs. The Compose wrapper then requires the pinned host `runsc` registration and a successful networkless live-container smoke before invoking Compose. Brokered Secret jobs and this setting are not supported on arm64.                                                                                                                                                                                                                                |
...
| `VERITY_TRANSCRIBE_MODEL`              | `whisper-1`                                                          | Model identifier sent to the transcription backend. Must match a model the configured provider serves.                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `VERITY_TRANSCRIBE_RESPONSE_FORMAT`    | `verbose_json`                                                       | OpenAI transcription response format. The default preserves segment timestamps needed to deduplicate overlapping chunks.                                                                                                                                                                                                                                                                                                                                                                                        |
| `VERITY_TRANSCRIBE_LANGUAGE`           | _(unset)_                                                            | Optional ISO-639-1 language hint sent to the transcription API.                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `VERITY_TRANSCRIBE_TIMEOUT_MS`         | `14400000`                                                           | Transcription request timeout (four hours) for long meeting recordings.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `VERITY_TRANSCRIBE_RETRIES`            | `12` (compose)                                                       | Connection-only retry attempts while the transcription backend is briefly unreachable. HTTP inference failures use the separate budget below.                                                                                                                                                                                                                                                                                                                                                                   |
| `VERITY_TRANSCRIBE_HTTP_RETRIES`       | `0` (compose)                                                        | Retry attempts after a real retriable HTTP response. The reference deployment does not repeat long-running inference failures.                                                                                                                                                                                                                                                                                                                                                                                  |
| `VERITY_TRANSCRIBE_RETRY_DELAY_MS`     | `5000` (compose)                                                     | Delay between transcription retry attempts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `VERITY_MEETING_CHUNK_SECONDS`         | `300` (compose)                                                      | Splits meeting audio into bounded five-minute windows transcribed sequentially. With the default context overlap, each request stays below the 400-second single-pass limit of OpenAI Whisper models; set to `0` only for backends that segment long audio themselves.                                                                                                                                                                                                                                          |
