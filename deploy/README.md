...
- Brokered Secret jobs are supported on **amd64 and arm64** and require the pinned gVisor host
  runtime. Install and verify it with [`deploy/gvisor`](gvisor/README.md), then set
  `VERITY_GVISOR_REQUIRED=1` so the Compose
...
| `VERITY_GVISOR_REQUIRED`               | `0`                                                                  | On amd64 or arm64, set to `1` on hosts that run Brokered Secret jobs. The Compose wrapper then requires the architecture-specific pinned host `runsc` registration and a successful networkless live-container smoke before invoking Compose.                                                                                                                                                                                                                                                                   |
