...
static void isolate_reads(const char *root, char **secret_paths, int secret_count) {
  ...
  const char *roots[] = {"/usr", "/bin", "/lib", "/lib64", "/etc"};
...
int main(int argc, char **argv) {
  if (argc == 2 && !strcmp(argv[1], "--probe")) {
    ...
    isolate_reads("/usr", NULL, 0);
    return 0;
  }
  if (argc < 9 || strcmp(argv[1], "--root") || strcmp(argv[3], "--cwd") ||
      strcmp(argv[5], "--loading")) {
    fputs("usage: verity-script-sandbox --root PATH --cwd PATH --loading isolated|dynamic [--secret PATH] -- COMMAND\n",
          stderr);
    return 126;
  }
  ...
  if (!strcmp(argv[6], "isolated")) isolate_reads(argv[2], &argv[8], (command - 7) / 2);
